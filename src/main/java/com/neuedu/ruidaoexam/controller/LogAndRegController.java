...
public class LogAndRegController {
	   ...
	   /*
		...
		public String loginsubmit(String username,String password,String check_code,
			...
			if(larservice.login(username, password) == 1) {
				request.getSession().setAttribute("role", "teacher");
				Teacher teacher =  (Teacher) larservice.getUser(username, password);
				request.getSession().setAttribute("id", teacher.getTeacherId());
				request.getSession().setAttribute("name", teacher.getName());
				return "1";
			}else if(larservice.login(username, password) == 2) {
				request.getSession().setAttribute("role", "student");
				Student student =  (Student) larservice.getUser(username, password);
		...
		public String getCurrentUser(HttpServletRequest request) {
			...
			json.put("id",session.getValue("id"));
